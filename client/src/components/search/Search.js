        ...
        const sortGroups = (items) => {
                return [...items].sort((a, b) => {
                        switch (sortKey) {
                                ...
                                case 'arrival':
                                        return (
                                                (a.outbound.scheduled_arrival_time || '').localeCompare(
                                                        b.outbound.scheduled_arrival_time || ''
                                                )
                                        );
                                case 'departure':
                                        return (
                                                (a.outbound.scheduled_departure_time || '').localeCompare(
                                                        b.outbound.scheduled_departure_time || ''
                                                )
                                        );
                                case 'duration':
                                        return (
                                                getFlightDurationMinutes(a.outbound) -
                                                getFlightDurationMinutes(b.outbound)
                                        );
                                default:
                                        return 0;
                        }
