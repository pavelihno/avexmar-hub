...
const docTypeOptions = Object.entries(ENUM_LABELS.DOCUMENT_TYPE).map(
...
const getAgeError = (type, birthDate) => {
  ...
  if (type === "ADULT" && age < 12)
    return VALIDATION_MESSAGES.PASSENGER.birth_date.ADULT;
  if (type === "CHILD" && (age < 2 || age > 12))
    return VALIDATION_MESSAGES.PASSENGER.birth_date.CHILD;
...
const PassengerForm = ({ passenger, onChange, onRemove, onSelectDocument }) => {
  ...
  const validate = () => {
    const errs = {};
    if (!data.lastName)
      errs.lastName = VALIDATION_MESSAGES.PASSENGER.last_name.REQUIRED;
    if (!data.firstName)
      errs.firstName = VALIDATION_MESSAGES.PASSENGER.first_name.REQUIRED;
    const ageErr = getAgeError(data.type, data.birthDate);
    if (ageErr) errs.birthDate = ageErr;
    if (!data.documentType)
      errs.documentType = VALIDATION_MESSAGES.PASSENGER.document_type.REQUIRED;
