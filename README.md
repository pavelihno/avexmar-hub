...
-   **pgbouncer** for connection pooling to the managed PostgreSQL instance.
-   **storage-cdn** for serving static assets uploaded by users.
...
1. A Linux host with Docker Engine installed.
...
3. Pull the production branch (or copy `docker compose.yml`, `Caddyfile`, monitoring config, and populated `.env` files) onto the host. Persist named volumes (`caddy_data`, `caddy_config`, `redis_data`, `loki_data`, `grafana_data`, `alloy_data`, `app_logs`, `app_storage`) on durable storage if you require backups.
